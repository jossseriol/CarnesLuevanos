...
# MFA desactivado: ningun rol debe pedir codigo de Google Authenticator.
MFA_ROLES = set()
...
def _stable_device_identity() -> str:
    """Return an identifier that survives computer and network-card renames."""
    if platform.system() == "Windows":
        try:
            ...
            if machine_guid:
                return f"windows|{str(machine_guid).strip().lower()}"
        except (OSError, ImportError):
            pass

    # On systems without a machine identifier, keep a private local token.
    data_root = os.getenv("LOCALAPPDATA") or os.getenv("XDG_DATA_HOME")
    token_dir = Path(data_root).expanduser() if data_root else Path.home() / ".local" / "share"
    token_path = token_dir / "CarnesLuevanos" / "device_id"
    try:
    ...
    try:
        ...
        # Last-resort fallback for read-only environments. Do not include the
        # computer name because renaming Windows caused legitimate lockouts.
        return f"fallback|{platform.system()}|{uuid.getnode()}"


def device_id() -> str:
    raw = f"carnes-luevanos-device-v2|{_stable_device_identity()}"
...
def reset_password_with_admin(target_username: str, admin_username: str, admin_password: str, new_password: str):
    """Restablece una contraseña con autorización administrativa y deja auditoría."""
    ensure_security_schema()
    target_username = str(target_username or "").strip()
    admin_username = str(admin_username or "").strip()
    if not target_username or not admin_username or not admin_password or not new_password:
        return {"ok": False, "message": "Completa todos los datos solicitados."}
    ...
    with connect() as c:
        target = c.execute(
            "SELECT id,username FROM usuarios WHERE lower(username)=lower(?)",
            (target_username,),
        ).fetchone()
        admin = c.execute(
            "SELECT id,username,password,rol,estado FROM usuarios WHERE lower(username)=lower(?)",
        ...
        if not target or not admin:
            _audit(c, admin["id"] if admin else None, "reset_password_denegado", f"objetivo={target_username}", False)
            return {"ok": False, "message": "Usuario o credenciales administrativas incorrectas."}

        role = str(admin["rol"] or "").strip().lower()
        admin_state = str(admin["estado"] or "activo").strip().lower()
        password_ok, upgraded = _verify_and_upgrade(admin["password"], admin_password)
        if role not in ADMIN_UNLOCK_ROLES or admin_state != "activo" or not password_ok:
            _audit(c, admin["id"], "reset_password_denegado", f"objetivo={target_username}", False)
            return {"ok": False, "message": "Credenciales administrativas incorrectas o sin autorización."}
        ...
        c.execute(
            """
            UPDATE usuarios
            SET password=?, password_cambiada=?, password_vence=?, requiere_cambio_password=0,
        ...
        c.execute(
            "UPDATE sesiones_usuario SET cerrada=?, motivo_cierre='restablecimiento administrativo' "
            "WHERE usuario_id=? AND cerrada IS NULL",
            (now.isoformat(timespec="seconds"), target["id"]),
        )
        _audit(c, admin["id"], "password_restablecida", f"objetivo={target['username']}", True)
        _audit(c, target["id"], "password_restablecida", f"por={admin['username']}", True)
        return {"ok": True, "message": f"La contraseña de {target['username']} fue restablecida correctamente."}
